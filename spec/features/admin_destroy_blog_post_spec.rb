...
feature 'Admin destroy Post Posts', js: true do
  scenario 'successfully', driver: :webkit do
    ...
    expect(page).not_to have_css('a', text: "Criado em: #{post.created_at.strftime('%B %d, %Y')}")
    expect(page).not_to have_css('a', text: "Atualizado em: #{post.updated_at.strftime('%B %d, %Y')}")
    expect(page).not_to have_css("img[src*='#{post.images.first.file.identifier}']")
